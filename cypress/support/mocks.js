...
Cypress.Commands.add("Summary", (option) => {
    if (option == 'true') {
        cy.route('GET', '**/insurance-validation', 'fixture:insurance-validation-true.json')
    }else if (option == 'false') {
        cy.route('GET', '**/insurance-validation', 'fixture:insurance-validation-false.json')
    }else if (option == 'fail') {
        cy.route({
            method: 'GET',
...
Cypress.Commands.add("InsuranceValidation", (option) => {
    if (option == 'true') {
        cy.route('POST', '**/insurance-validation', 'fixture:insurance-validation-true.json')
    }else if (option == 'false') {
        cy.route('POST', '**/insurance-validation', 'fixture:insurance-validation-false.json')
    }else if (option == 'fail') {
        cy.route({
            method: 'POST',
...
Cypress.Commands.add("UserIdentityValidate", (option) => {
    if (option == 'PASS') {
        ...
        cy.route('POST', '**/user-identity', 'fixture:user-identity-validate-200-error.json')
...
Cypress.Commands.add("ReadChannels", (channels) => {
    if (channels== 'TRUE') {
        cy.route('GET', '**/read-active-channels', 'fixture:read-active-channels-true.json')
    } else if (channels== 'FALSE') {
        cy.route('GET', '**/read-active-channels', 'fixture:read-active-channels-false.json')
...
Cypress.Commands.add("CustomerAccount", (cat) => {
    if (cat) {
...
Cypress.Commands.add("Sqs", (success) => {
    if (success == 'success') {
        cy.route('POST', '**/sqs-cda-documents', 'fixture:sqs-cda-documents-success.json')
    } else {
        cy.route({
            method: 'POST',
...
Cypress.Commands.add("GetDocs", (docsType) => {
    if (docsType == 'Empleado') {
        cy.route('POST', '**/get-documents-to-charge', 'fixture:get-documents-all.json')
    } else if (docsType == 'cc') {
        cy.route('POST', '**/get-documents-to-charge', 'fixture:get-documents-only-cc.json')
    }
    else if (docsType == 'no cc') {
        cy.route('POST', '**/get-documents-to-charge', 'fixture:get-documents-no-cc.json')
