...
Cypress.Commands.add("ValidateErrorMessage", (errorMessage) => {
    cy.get('mat-error[role="alert"]', { timeout: 20000 }).should('contain', errorMessage)
})
...
Cypress.Commands.add("ValidateCard", (cardType, visible) => {
    if (cardType == ('PRO') && visible) {
        cy.get('.mat-radio-label', { timeout: 10000 }).contains('PRO').should('be.visible')
    }
    else if (cardType == ('SIMPLE') && visible) {
        cy.get('.mat-radio-label', { timeout: 10000 }).contains('SIMPLE').should('be.visible')
    }
    else if (cardType == ('PRO') && !visible) {
        cy.get('.mat-radio-label', { timeout: 10000 }).contains('PRO').should('be.not.visible')
    }
    else if (cardType == ('SIMPLE') && !visible) {
        cy.get('.mat-radio-label', { timeout: 10000 }).contains('SIMPLE').should('be.not.visible')
    }
})
...
Cypress.Commands.add("FillSendAddress", (pause) => {
    ...
    //  cy.get('@direccion').type('direccion nueva')
    cy.Pause(pause)
