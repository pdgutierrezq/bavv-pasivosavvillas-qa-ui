...
Cypress.Commands.add("FundingCdt", (funding, rend, pause) => {
  if (funding == 'PSE') {
    cy.get(
        '[formcontrolname="accountSettingFirstGroup"]>.mat-radio-button:contains("Desde otro banco")').click()
  }

  if (rend == 'ACCOUNT') {
    cy.get(
        '[formcontrolname="accountSettingSecondGroup"]>.mat-radio-button').first().click()
  }
  cy.Pause(pause)
  cy.get('#SubmitAccountSettingsForm').click()
})

Cypress.Commands.add("ConfigCdt",
    (pause = Cypress.env().screen.config.cdt.pause) => {
...
Cypress.Commands.add("TypeAndValidate", (amount, term, rate) => {
  ...
  if (term % 30 == 0) {
    if (term == 120 || term == 240 || term == 300 || term == 420 || term == 480
        || term == 600 || term == 660 || term == 780 || term == 840 || term
        == 960) {
      refund = 'Cada mes';
    } else if (term == 180) {
...
Cypress.Commands.add("ValidateRend", (amount, term, refund) => {
  ...
  cy.get(".rate").then(function ($elem) {
    ...
    cy.get(".profit").then(function ($elem) {
      var r = $elem.text().replace("$", "").replaceAll(".", "").replace(",",
          ".").replace(" ", "");
      ...
      } else if (r.substring(0, r.length - 2) == rendimientosString.substring(0,
          rendimientosString.length - 2)) {
        r = r.substring(0, r.length - 2);
        console.log('r: ', r)
        rendimientosString = rendimientosString.substring(0,
            rendimientosString.length - 2);
    ...
    cy.get(".retention").then(function ($elem) {
      ...
      cy.get(".earnings").then(function ($elem) {
        var netoObtenido = $elem.text().replace("$", "").replaceAll(".",
            "").replace(",", ".").replace(" ", "");
        var rSplit = $elem.text().replace("$", "").replaceAll(".", "").replace(
            ",", ".").replace(" ", "").split('.')[0];

        if (rSplit == netoSplit) {
          expect(rSplit).to.equal(netoSplit)
        } else if (netoSplit.substring(0, netoSplit.length - 2)
            == rSplit.substring(0, rSplit.length - 2)) {
...
Cypress.Commands.add("AccountConfiguration",
    (pause = Cypress.env().screen.cdt.account.pause) => {
